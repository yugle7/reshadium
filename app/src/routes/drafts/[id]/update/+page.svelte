<script>
	...
	const { id, title, categories, condition, notes, answer, proof } = data.draft;
	const draft = { title, categories, condition, notes, answer, proof };
	$params = structuredClone(draft);

	$: disabled = JSON.stringify($params) === JSON.stringify(draft) || !$params.categories.length;
	$loaded = true;
</script>
...
{#if $screen}
	<a class="top-right" href="/drafts/{id}">
	...
	<div class="page">
		<form method="post" class="col content-900 padding-20 gap-40" use:enhance>
			...
			<div class="row gap-15 center right">
				{#if !$screen}
					<a class="link" href="/drafts/{id}">Отменить</a>
				{/if}
